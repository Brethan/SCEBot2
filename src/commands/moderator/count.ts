...
export default class Count extends Command {
	...
	async textCommand(message: Message, args: string[]): Promise<MessageCreateOptions> {
		...
		const programName: string = aliasToProgram.get(alias);
		const programRole: Role = message.guild.roles.cache.find(r => r.name === programName);
		const membersInProgram: number = programRole.members.size;
